/********************************************************************************************
NAME:    data_loading
PURPOSE: Load data model DFNB2 database and assign keys
...
---   ----------  -------   -----------------------------------------------------------------
1.0   07/19/2020  KIZYMATZEN   1. Script creation
...
-- 14) Create Transaction Type Dimension Table
...
-- 15) Create Transaction Real Time Fact Table
...
SELECT DISTINCT 
  ...
  --16) Create Loan Period Dimension Table

DROP TABLE t_loan_period_dim;

CREATE TABLE t_loan_period_dim
(loan_period_id INT IDENTITY(1, 3) PRIMARY KEY NOT NULL
, loan_open_date DATE NOT NULL
, loan_period_yr AS CASE
                        WHEN loan_open_date = NULL
                        THEN 0
                        ELSE DATEDIFF(year, loan_open_date, GETDATE())
                    END
);

INSERT INTO t_loan_period_dim(loan_open_date)
SELECT DISTINCT 
       open_date
  FROM dbo.stg_p1 AS sp;

  --17) Create Loan Current Dimension Table

DROP TABLE t_loan_curr_amt_dim;

CREATE TABLE t_loan_curr_amt_dim
...
INSERT INTO t_loan_curr_amt_dim(acct_id
                               , loan_amt)
SELECT DISTINCT 
       acct_id
     , loan_amt
  ...
  --18) Create Loan Amount Fact Table

 
DROP TABLE t_loan_amt_fact;

CREATE TABLE t_loan_amt_fact
(loan_amt_id    INT IDENTITY(1, 2) PRIMARY KEY NOT NULL
, acct_id        INT NOT NULL
, branch_id      SMALLINT NOT NULL
...
SELECT DISTINCT 
  ...
  FROM dbo.stg_p1 AS p1
       INNER JOIN
       t_loan_curr_amt_dim AS l ON l.loan_curr_amt_id = p1.acct_id
       INNER JOIN
       t_loan_period_dim AS p ON p.loan_period_id = p1.acct_id;

-- 19) Create Loan Bridge Table

DROP TABLE t_loan_brg;

CREATE TABLE t_loan_brg
(loan_brg_id      INT IDENTITY(1, 2) PRIMARY KEY NOT NULL
, loan_curr_amt_id INT NOT NULL
, loan_amt_id      INT NOT NULL
, loan_period_id   INT NOT NULL
);

INSERT INTO t_loan_brg(loan_curr_amt_id
                     , loan_amt_id
                     , loan_period_id)
SELECT DISTINCT 
       loan_curr_amt_id
     , loan_amt_id
     , loan_period_id
  FROM t_loan_amt_fact AS a
       INNER JOIN
       t_loan_curr_amt_dim AS l ON l.loan_curr_amt_id = a.loan_amt_id
       INNER JOIN
       t_loan_period_dim AS p ON p.loan_period_id = a.loan_amt_id;
